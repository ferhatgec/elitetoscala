...
fn create_and_write(path: &std::path::Path, generated: String) {
    let mut file = match std::fs::File::create(path) {
        Err(why) => panic!("elitetoscala: couldn't write to {}: {}", path.display(), why),
        Ok(file) => file
    };
...
fn main() {
    ...
    if cli_arguments.len() < 2 {
        println!("elitetoscala - create scala source from elite\n\
                 ----------\n\
    ...
    let y = parse::elite_scala::parse(x);
    let z = format!("{}_out.scala", cli_arguments.get(1).unwrap()).as_str().to_owned();
